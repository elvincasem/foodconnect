...
$(function (){
	...
	var uploadURI = $('#form_upload').attr('action');
	...
	function listFilesOnServer(){
		...
		$.getJSON(uploadURI, function(data){
			$.each(data, function(index, element){
				...
				//items.push('<li>' + element + '<a href="#" data-file="' + element + '" class="remove-file"><i class="fa fa-times"></i></a></li>');
			});
...
function uploaddish(){
	var inputFile = $('input[name=dishphoto]');
	var uploadURI = $('#form_uploadbestseller').attr('action');
	var progressBar = $('#progress-bar-dish');
	...
	$('#uploadbtn').on('click',function(event){
		...
		if (fileToUpload != 'undefined'){
			var formData = new FormData();
			formData.append("dishphoto",fileToUpload);
...
function saverestaurant(){
	...
	var rlogo = reslogo.replace(/^.*\\/,"");
	var rcover = rescoverphoto.replace(/^.*\\/,"");
	
	$.ajax({
		url: "functions/saverestaurant",
		type: 'post',
		data: {restaurantname:restaurantname, street:resstreet, barangay:resbarangay, municipality:resmunicipality, contact:rescontact, email:resemail, shdayfrom:resshfromday, shdayto:resshtoday, shtimefrom:resshfromtime, shtimeto:resshtotime, logo:rlogo, coverphoto:rcover},
	...
	var inputlogo = $('input[name=reslogo]');
	var inputcover = $('input[name=rescoverphoto]');
	var uploadURI = $('#form_upload').attr('action');
	var progressBar = $('#progress-bar');
	...
	$('#savebtn').on('click',function(event){
		var fileToUploadlogo = inputlogo[0].files[0];
		var fileToUploadcover = inputcover[0].files[0];
		console.log(fileToUploadlogo);
		console.log(fileToUploadcover);
		
		if (fileToUploadlogo != 'undefined'){
			var formData = new FormData();
			formData.append("reslogo",fileToUploadlogo);
		...
		if (fileToUploadcover != 'undefined'){
			var formData = new FormData();
			formData.append("rescoverphoto",fileToUploadcover);
			...
			$.ajax({
				...
				xhr: function(){
					var xhr = new XMLHttpRequest();
					xhr.upload.addEventListener("progress-bar-two", function(event) {
						if (event.lengthComputable){
							var percentComplete = Math.round((event.loaded / event.total) * 100);
							$('.progress-bar-two').show();
							progressBartwo.css({width: percentComplete + "%"});
							progressBartwo.text(percentComplete + '%');
						};
					}, false);
	...
	function listFilesOnServer(){
		...
		$.getJSON(uploadURI, function(data){
			$.each(data, function(index, element){
				items.push('<li style="display: none;"><input type="text" value="' + element + '"></li>');
			});
...
function savedish(){
	...
	var ddescription = document.getElementById("dishdescription").value;
	var dimage = document.getElementById("dishphoto").value;
	
	var dishimage = dimage.replace(/^.*\\/,"");
	
	$.ajax({
		url: "functions/savedish",
		type: 'post',
		data: {rid:restaurantid, dishname:dname, price:dprice, description:ddescription, image:dishimage},
		success: function(response) {
			console.log(response);
			window.location.reload();;
		}
...
function savetype(){
	...
	$.ajax({
		url: "functions/savetype",
		type: 'post',
		data: {rid:restaurantid, type:restauranttype},
		success: function(response) {
			console.log(response);
			window.location.reload();;
		}
...
/////////////////////////////////////////////////////////////UPDATE DATA////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////UPDATE DATA////////////////////////////////////////////////////
function disableupdatetype(){
	
	$('#savetypebtn').prop("disabled", false);
	$('#updatetypebtn').prop("disabled", true);
	
}

function disableupdateres(){
...
function updaterestaurant(){
	...
	$.ajax({
		...
		type: 'post',
		data: {rid:restaurantid, restaurantname:restaurantname, street:resstreet, barangay:resbarangay, municipality:resmunicipality, contact:rescontact, email:resemail, shdayfrom:resshfromday, shdayto:resshtoday, shtimefrom:resshfromtime, shtimeto:resshtotime, logo:rlogo, coverphoto:rcover},
		success: function(response) {
			console.log(response);
			window.location.reload();;
		}
...
function deletetype(typeid){
	...
	var r = confirm("Are your sure you want to delete this?");
    if (r == true) {
		var person = prompt("Please enter Administrator Password");
    
		if (person == 'superadmin') {
			$.ajax({
                    url: 'functions/deletetype/'+typeid,
                    type: 'post',
                    success: function(response) {
						window.location.reload();
                    }
